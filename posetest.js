...
class Robot {
    constructor(axes=6, workFrame='tool', tool=[0,0,0,0,0,0], work=[0,0,0,0,0,0]) {
        this.axes = axes;
        this.workFrame = workFrame;
        this.tool = tool;
        this.work = work;
    ...
    Connect(address) {
        ...
        this.socket.on('open', function open() { UpdateStatus(); });
    }
...
function shake(robot) {
    robot.Speed(60);
    robot.SendMessage('SetTRF(0,0,-50,0,0,0)');
    robot.SendMessage('SetBlending(90)');
    ...
    robot.SendMessage('SetTRF(0,0,0,0,0,0)');
}
